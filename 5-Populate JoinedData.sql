...
with  
  ...
  w as (select date_diff(datetime, timestamp('1970-01-01 00:00:00'), second) as wt, unit_id from LETestDataset1.wialon order by unit_id, wt),
  mfwjoined as (
    select f.machineid, f.ft, w.wt, w.unit_id
    from f
    inner join LETestDataset1.MachineIdLink as fw on fw.machineId = cast(f.machineId as STRING)
  ...
  mtwjoined as (
    ...
    from t
    inner join LETestDataset1.MachineIdLink as fw on fw.machineId = cast(t.machineId as STRING)
...
select
  ...
  f.WHEEL_SLIP ,
  f.WORK_ON 
from LETestDataset1.fendt as f, LETestDataset1.wialon as w, fwjoin
